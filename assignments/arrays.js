...
//Help the dealer find out which car has an id of 33 
//by logging the car's year, make, and model in the console log provided to you below:

for (item of inventory) {
  if (item.id === 33) {
    console.log(item.car_year, item.car_make, item.car_model);
  }
}
...
let BMWAndAudi = [];
for (item of inventory) {
  if (item.car_make == "Audi") {
    BMWAndAudi.push(item)
  } else if (item.car_make == "BMW") {
    BMWAndAudi.push(item)
  }
}
